...
def create_initial_population(size, lower_bound, upper_bound):
    ...
    for _ in range(size):
        individual = (
            random.uniform(lower_bound, upper_bound),  # a
            random.uniform(lower_bound, upper_bound),  # b
            random.uniform(lower_bound, upper_bound),  # c
        )
...
def genetic_algorithm(population_size, lower_bound, upper_bound, generations, mutation_rate):
    ...
    axs[0].scatter(
        [final_population.index(best_individual)], [best_individual[0]], color="cyan", s=100, label="Best Individual a"
    )
    ...
    ax.fill_between(
        generations_list, min_fitness_values, max_fitness_values, color="gray", alpha=0.5, label="Fitness Range"
    )
    ax.set_xlabel("Generation")
